...
def session_type_col(df: DataFrame) -> DataFrame:
    df["session_type"] = df.apply(
        lambda x: get_session_type(x["indoor_session"], x["outdoor_session"]),
        axis=1,
    )
    return df
...
def max_v_grade_cols(df: DataFrame) -> DataFrame:
    df["max_indoor_v_grade"] = df.apply(
        lambda x: get_max_v_grade(
            x["session_type"], x["max_v_grade"], "indoor"
        ),
        axis=1,
    )
    df["max_outdoor_v_grade"] = df.apply(
        lambda x: get_max_v_grade(
            x["session_type"], x["max_v_grade"], "outdoor"
        ),
        axis=1,
    )
    return df
